# Issue 9: Perform configuration review of the `crypto-scout-test` project

The first version of the `crypto-scout-test` project has been done now. Let's perform the configuration review to be
...
- As the `expert devops engineer` perform configuration review of the `crypto-scout-test` project and verify if this is
...
- As the `expert devops engineer` recheck your proposal and make sure that they are correct and haven't missed any
  important points.
- As the `expert technical writer` update the `9-perform-configuration-review.md` file with your resolution.